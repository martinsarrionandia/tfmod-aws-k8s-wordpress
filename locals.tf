locals {
  ...
  middleware-cdn-rewrite         = "${kubernetes_namespace_v1.this.metadata[0].name}-${local.middleware-cdn-rewrite-name}@kubernetescrd"
  middleware-cache-control-name  = "${var.release-name}-cache-control"
  middleware-cache-control       = "${kubernetes_namespace_v1.this.metadata[0].name}-${local.middleware-cache-control-name}@kubernetescrd"
